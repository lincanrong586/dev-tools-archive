...
// 兼容旧版 cut_size_um：
// - 当 cut_size_um > 0 时，说明调用方仍在使用“正方形窗口”参数；
// - 否则使用新的宽/高参数。
int32_t EffectiveCutWidth(const GenerateConfig& cfg) {
  return cfg.cut_size_um > 0 ? cfg.cut_size_um : cfg.cut_width_um;
}

int32_t EffectiveCutHeight(const GenerateConfig& cfg) {
  return cfg.cut_size_um > 0 ? cfg.cut_size_um : cfg.cut_height_um;
}
...
uint32_t ChoosePolygonCount(const GenerateConfig& cfg, uint32_t total_points, std::mt19937& rng) {
  const uint32_t min_polygon_points = std::max(1u, cfg.min_polygon_points);
  const uint32_t max_polygon_points = std::max(min_polygon_points, cfg.max_polygon_points);

  const uint32_t min_polygons =
      std::max(1u, (total_points + max_polygon_points - 1) / max_polygon_points);
  const uint32_t max_polygons =
      std::max(1u, total_points / min_polygon_points);

  const uint32_t preferred_by_avg =
      std::max(1u, total_points / std::max(1u, cfg.points_per_polygon));
  ...
  const uint32_t jitter = std::max(1u, clamped_preferred / 10);
  const uint32_t low = ClampU32(clamped_preferred > jitter ? clamped_preferred - jitter : 1u,
...
std::vector<uint32_t> GeneratePolygonPointCounts(const GenerateConfig& cfg, uint32_t polygon_count,
                                                 uint32_t total_points, std::mt19937& rng) {
  const uint32_t min_polygon_points = std::max(1u, cfg.min_polygon_points);
  const uint32_t max_polygon_points = std::max(min_polygon_points, cfg.max_polygon_points);
  ...
  for (uint32_t i = 0; i < polygon_count; ++i) {
    ...
    const uint32_t min_extra_here =
        remaining > rest * max_extra_per_polygon ? remaining - rest * max_extra_per_polygon : 0;
...
static long long ClampLL(long long v, long long low, long long high) {
...
static Polygon GenerateMaskLikePolygon(long long cx, long long cy, long long radius_nm, uint32_t points,
                                      std::mt19937& rng) {
  const uint32_t control = std::max(8u, std::min(32u, points / 10));
  ...
  for (uint32_t i = 0; i < control; ++i) {
    ...
    const double rr = static_cast<double>(radius_nm) * (0.65 + 0.35 * jitter01(rng));
...
static PolygonSet GenerateMaskLikePolygonSet(const GenerateConfig& cfg, std::mt19937& rng) {
  ...
  window.x0 = x0;
  window.y0 = y0;
  window.x1 = x0 + std::max(0LL, cut_w_nm - 1);
  window.y1 = y0 + std::max(0LL, cut_h_nm - 1);

  const uint32_t total_points = ChooseTotalPoints(cfg, rng);

  const uint32_t desired_pp = std::max(300u, cfg.points_per_polygon);
  const uint32_t rough_polygons = std::max(1u, total_points / desired_pp);
  const uint32_t jitter = std::max(1u, rough_polygons / 10);
  std::uniform_int_distribution<uint32_t> poly_count_dist(rough_polygons > jitter ? rough_polygons - jitter : 1u,
  ...
  local.min_polygon_points = std::max(200u, cfg.min_polygon_points);
  local.max_polygon_points = std::max(local.min_polygon_points, std::max(2000u, cfg.max_polygon_points));
  ...
  const long long cx_low = ClampLL(window.x0 - margin, 0, std::max(0LL, region_nm - 1));
  const long long cx_high = ClampLL(window.x1 + margin, 0, std::max(0LL, region_nm - 1));
  const long long cy_low = ClampLL(window.y0 - margin, 0, std::max(0LL, region_nm - 1));
  const long long cy_high = ClampLL(window.y1 + margin, 0, std::max(0LL, region_nm - 1));
...
PolygonSet GeneratePolygonSet(const GenerateConfig& cfg) {
  // 先确定当前实际生效的窗口宽高。
  //
  // 如果调用方传入了旧参数 cut_size_um，则这里会自动退化成“正方形窗口”。
  const int32_t cut_width_um = EffectiveCutWidth(cfg);
  const int32_t cut_height_um = EffectiveCutHeight(cfg);
  std::mt19937 rng(cfg.seed);
  const uint32_t total_points = ChooseTotalPoints(cfg, rng);
  const uint32_t polygon_count = ChoosePolygonCount(cfg, total_points, rng);
  const std::vector<uint32_t> point_counts =
      GeneratePolygonPointCounts(cfg, polygon_count, total_points, rng);
  ...
  // 在窗口范围内均匀采样点坐标：
  // - x 方向落在 [x0, x0 + cut_width_um)
  // - y 方向落在 [y0, y0 + cut_height_um)
  ...
  PolygonSet ps;
  ps.reserve(point_counts.size());
  for (const uint32_t point_count : point_counts) {
    Polygon poly;
    poly.reserve(point_count);
    for (uint32_t j = 0; j < point_count; ++j) {
      // 这里生成的是“用于编码压力测试的点集”，
      // 因此只要求点落在窗口范围内，不额外强制闭环或边界有序。
      Point p;
      p.x = x0 + dx(rng);
      p.y = y0 + dy(rng);
      poly.push_back(p);
